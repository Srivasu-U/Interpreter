...
import (
	...
	"Interpreter/monkey-v4/src/parser"
	"testing"
)
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong params. Parameters=%+v", fn.Parameters)
	}
...
func TestFunctionApplication(t *testing.T) {
	tests := []struct {
		...
		{"let double = fn(x) { x * 2; }; double(5);", 10},             // expression as return value
		{"let add = fn(x, y) { x + y; }; add(5, 5);", 10},             // multiple params
		{"let add = fn(x, y) { x + y; }; add(5 + 5, add(5, 5));", 20}, // functions as params
		{"fn(x) { x; }(5)", 5},                                        // higher order function
	}
