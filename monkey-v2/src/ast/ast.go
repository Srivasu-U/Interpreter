...
import (
	"Interpreter/monkey-v2/src/token"
	"bytes"
	"strings"
)
...
type ExpressionStatement struct { // We have expression statements so we can add this to Program.Statements
...
type IfExpression struct {
	// If is technically an expression and not a statement as it evaluates into a value: true, false or even return
	// Ex: let foobar = if (x > y) { x } else { y } evalutes into either x or y and feeds into a let statement
